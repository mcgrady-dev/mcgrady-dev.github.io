...
Java是在JVM虚拟机的内存环境中运行的，内存分为三个区：堆、栈和方法区。

- 栈（Stack）：是简单的数据结构，程序运行时系统自动分配，使用完毕后自动释放。优点：速度快。
- 堆（Heap）：用于存放由 new 创建的对象和数组。在堆中分配的内存，一方面由JVM自动垃圾回收器来管理，另一方面还需要程序员提高修养，来防止内存泄露问题。
- 方法区（method）：又叫静态区，跟堆一样，被所有的线程共享。方法区包含的所有class和static变量。
